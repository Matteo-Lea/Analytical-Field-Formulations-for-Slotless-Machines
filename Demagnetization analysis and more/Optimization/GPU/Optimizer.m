...
% ----------------------------------------------------------------------- %
function REP = Optimizer(params,params_mach,Var_max,Var_min,Int_min,Int_max,Const,adm,Constraints)
...
%     params_mag.In_Out = sign(rand(el,1).*(Int_max(1,1)-Int_min(1,1))+Int_min(1,1)); % 1 for inrunner -1 for outrunner (this shouldn't vary in the optimizatio n process)
%     params_mag.p = randi([Int_min(1,2),Int_max(1,2)],el,1); % pole pairs
...
%     params_geo.R_sleeve = (x(:,3) - x(:,10).*x(:,7)- x(:,10).*x(:,6)- x(:,10).*x(:,5)- x(:,10).*x(:,8)).*(params_geo.R_i~=0&params_geo.R_i~=Inf); % magnets array radius (facing magnets support)[m]
    ...
    m_PM = 500; % number of harmonics or series components for the magnetization functions
    r_dis = 500; % radial discretization of magnets region

    var_max = Var_max;
    var_min = Var_min;
    
    p_f = 10; % penalty factor
    
    [M_r_n,M_theta_n] = Magnetization(el,m_PM,x,Const);
    [E_n,T_avg,t_spec,Magnets_weight,Joule,Tot_weight,Weighted_Perc_demag] = Field_Solution_Demag(el,m_PM,r_dis,x,params_geo,M_r_n,M_theta_n,params_mach,Const);
    f_lim = [ T_avg, sqrt(sum(E_n.^2,2)),Weighted_Perc_demag];
    f_lim_1 = [ max(abs(f_lim(:,1)-Constraints(1))-eps*Constraints(1),0), max(abs(f_lim(:,2)-Constraints(2))-eps*Constraints(2),0)];
    f_lim_2 = max(abs(f_lim(:,3)-Constraints(3))-eps*Constraints(3),0);
%     f = [ -t_spec,Tot_weight, sum(f_lim_1,2)];
%     f = [ -t_spec,Tot_weight, abs(T_avg-1600)];
%     f = [ -T_avg, Tot_weight];
%     f = [ -t_spec+p_f*Weighted_Perc_demag,Tot_weight+p_f*Weighted_Perc_demag];
    f = [ -t_spec,Tot_weight];
    real = [ -t_spec,Tot_weight];
%     weight_f = [2 1];
...
%     f = [abs(params_mach.B_sat-B_bi), abs(params_mach.B_sat-B_sleeve)];
    ...
    while ~stopCondition
        ...
        if gen == 1  
            ...
            for ii = 1:int
                best_ind = (POS(:,end-int+ii)==PBEST(:,end-int+ii)); % vector holding ones for the particles located at their best position so far adnzero elsewhere
...
%                 rand_int = integer_mat{ii}(find(rand(el,1)<=Intervals));
    ...
    %     params_geo.R_i(params_geo.In_Out==-1) = params_geo.R_i(params_geo.In_Out==-1).*1./randi([0,1],nnz(params_geo.In_Out==-1),1); % non-magnetic support set for some out-runner topologies (randomly)
        ...
        [E_n,T_avg,t_spec,Magnets_weight,Joule,Tot_weight,Weighted_Perc_demag] = Field_Solution_Demag(el,m_PM,r_dis,POS,params_geo,M_r_n,M_theta_n,params_mach,Const);
        f_lim = [ T_avg, sqrt(sum(E_n.^2,2)),Weighted_Perc_demag];
        f_lim_1 = [ max(abs(f_lim(:,1)-Constraints(1))-eps*Constraints(1),0), max(abs(f_lim(:,2)-Constraints(2))-eps*Constraints(2),0)];
        f_lim_2 = max(abs(f_lim(:,3)-Constraints(3))-eps*Constraints(3),0);
%         f = [ -t_spec,Tot_weight, sum(f_lim_1,2)];
