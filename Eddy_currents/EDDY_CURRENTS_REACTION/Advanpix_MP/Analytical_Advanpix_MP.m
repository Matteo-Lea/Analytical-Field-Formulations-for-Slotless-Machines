...
% Inverter_star_3f_multiprecision
Inverter_star_3f
...
% m_J = mp('3'); % total number of harmonics better to be limited to 7 at most. 
...
tau = 1i^(3/2)*sqrt(cond*mu_r*mu_0*H*speed_radsec*p);
...
J_r = besselj(m,mp(R_r*tau));
Y_r = bessely(m,mp(R_r*tau));
J1_r = -tau.*(besselj(m+1,mp(R_r*tau))-besselj(m-1,mp(R_r*tau)))./2;
Y1_r = -tau.*(bessely(m+1,mp(R_r*tau))-bessely(m-1,mp(R_r*tau)))./2;
J_r(isnan(J_r)) = 0; 
J1_r(isnan(J1_r)) = 0; 
Y_r(isnan(Y_r)) = 0; 
Y1_r(isnan(Y1_r)) = 0;

J_m = besselj(m,mp(R_m*tau));
Y_m = bessely(m,mp(R_m*tau));
J1_m = -tau.*(besselj(m+1,mp(R_m*tau))-besselj(m-1,mp(R_m*tau)))./2;
Y1_m = -tau.*(bessely(m+1,mp(R_m*tau))-bessely(m-1,mp(R_m*tau)))./2;
J_m(isnan(J_m)) = 0; 
J1_m(isnan(J1_m)) = 0; 
Y_m(isnan(Y_m)) = 0; 
Y1_m(isnan(Y1_m)) = 0;
...
C = ((R_r.*Y1_r - Y_r.*m + R_r.*Y1_r.*(R_i/R_r).^(2.*m) + Y_r.*m.*(R_i/R_r).^(2.*m)).*COMM);
D = -((J1_r.*R_r - J_r.*m + J1_r.*R_r.*(R_i/R_r).^(2.*m) + J_r.*m.*(R_i/R_r).^(2.*m)).*COMM);

% Coefficients form stationary field solution
...
P_loss = conj((C.*J1_m+D.*Y1_m)); % H_theta* contribution to the loss equation
...
if contains(plotting, "yes")
    ...
    % t = t(1:round(length(t)/2));
    theta = 0;

    J_m = besselj(m,mp(r*tau));
    Y_m = bessely(m,mp(r*tau));
    J_m(isnan(J_m)) = 0;  
    Y_m(isnan(Y_m)) = 0; 
    shift = - 2*(t(2)-t(1));
