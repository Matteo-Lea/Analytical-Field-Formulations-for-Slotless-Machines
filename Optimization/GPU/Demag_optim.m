...
% ex-opt-1
l_a = 0.2033; % active lenght [m]
R_s = 0.15; % stator radius [m]
R_wi = R_s; % winding radius [m]
R_w = R_s-0.0078; % winding radius [m]
R_m = R_w-0.0015; % magnets array outer radius [m]
R_r = R_m-0.0081; % outer rotor radius (inner magnets radius) [m]
R_se = 1.01*R_s; % outer stator radius [m]
R_i = R_r; % Iron boundary radius facing the magnets
R_ie = R_i-sign(R_w-R_m)*0.1*R_i;
g = R_w-R_m; % air-gap thickness [m]
...
R_2 = R_s/2+R_se/2; % mid-stator radius
p = 17;
alpha_p = 0.6938; % mid-magnet to pole ratio [-]
...
% B_r = 1.3675; % remanent flux density [T] (mid-magnet)
% B_rs = 1.3271; % remanent flux density [T] (side-magnets)
...
B_knee = B_int;
% alpha_p = 0.526; % mid-magnet to pole ratio [-]
% if Halbach_1 == 2 
...
r_m = linspace(R_m,R_r,100)'; % radius at which the flux density is to be evaluated
...
if alpha_p~=0
    ...
    theta_1 = linspace(-alpha_p*pi/(2*p),alpha_p*pi/(2*p),100);
...
%     Weighted_vol_demag_perc_1 = sum(sum((B_dem_1 > 0).*(B_dem_1*mu_r/mu_rc)./B_r))/numel(B_dem_1)*100;
    ...
    if  mod(Halbach_1,2)==0
        ...
        Vol_mag3 = Vol_mag2;
        theta_2 = linspace(-pi/(2*p),-alpha_p1*pi/(2*p),100);
...
%         theta_3 = linspace(alpha_p1*pi/(2*p),pi/(p)-alpha_p1*pi/(2*p),1000);
        theta_3 = linspace(alpha_p1*pi/(2*p),pi/(2*p),100);
        theta_3 = theta_3(2:end-1);
        Theta_3 = repmat(theta_3,m_PM+1,1);
