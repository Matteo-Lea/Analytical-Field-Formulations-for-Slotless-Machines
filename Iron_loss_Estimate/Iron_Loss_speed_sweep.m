...
m_PM = 10; % number of harmonics or series components no need to increase 
           % it more than this
x = 0:1:m_PM;
n = 2*x+1; % existing harmonic series components (n=1,3,5,...)
...
%% Magnetization distribution two-segmetn Halbach array

% PARALLEL MAGNETIZED PM
% Coefficients for defining the magnetization distribution
% Mid-magnets coefficients
M_1_0 = sin((n*p-1)*alpha_p*pi/(2*p))./((n*p-1)*alpha_p*pi/(2*p));
M_2_0 = sin((n*p+1)*alpha_p*pi/(2*p))./((n*p+1)*alpha_p*pi/(2*p));
% Side-magnets coefficients
M_1_1 = cos((n*p-1)*alpha_p*pi/(2*p)+pi/(2*p))./((n*p-1)*alpha_p*pi/(2*p));
M_2_1 = cos((n*p+1)*alpha_p*pi/(2*p)-pi/(2*p))./((n*p+1)*alpha_p*pi/(2*p));


if p==1
   M_1_0(1) = 1;
   M_1_1(1) = sin(pi/2*p)*(pi/(2*p)-alpha_p*pi/(2*p));  
end
...
M_r_n_par_side = B_rs/mu_0*alpha_p*(M_1_1-M_2_1);
M_theta_n_par_side = -B_rs/mu_0*alpha_p*(M_1_1+M_2_1);

% magnetization reconstruction from harmonics components (not needed)
M_theta_par_mid = M_theta_n_par_mid'.*sin(n'*p.*Theta);
M_r_par_mid = M_r_n_par_mid'.*cos(n'*p.*Theta);
M_theta_par_mid = sum(M_theta_par_mid,1);
M_r_par_mid = sum(M_r_par_mid,1);

M_r_par_side = M_r_n_par_side'.*cos(n'*p.*Theta);
M_r_par_side = sum(M_r_par_side,1);
M_theta_par_side = M_theta_n_par_side'.*sin(n'*p.*Theta);
M_theta_par_side = sum(M_theta_par_side,1);

M_r_par = (M_r_n_par_mid+M_r_n_par_side)'.*cos(n'*p.*Theta);
M_theta_par = (M_theta_n_par_mid+M_theta_n_par_side)'.*sin(n'*p.*Theta);

% CYLINDRICALLY MAGNETIZED PM
% Coefficients for defining the magnetization distribution
M_r_n = 4*B_r*alpha_p*sin(n*alpha_p*pi/2)./(mu_0*n*alpha_p*pi);
M_theta_n = -4*B_rs*alpha_p*cos(n*alpha_p*pi/2)./(mu_0*n*alpha_p*pi);

% magnetization reconstruction from harmonics components (not needed)
M_r = M_r_n'.*cos(n'*p.*Theta);
M_theta = M_theta_n'.*sin(n'*p.*Theta);
M_r = sum(M_r,1);
M_theta = sum(M_theta,1);


    
M_r_n = M_r_n_par_mid+M_r_n_par_side;
M_theta_n = M_theta_n_par_mid+M_theta_n_par_side;
...
A_zm_n = -mu_0*(n*p.*M_r_n+M_theta_n)./(1-(n*p).^2);
...
% K_B_n = (((A_zm_n-n*p.*A_zm_n+mu_0*M_theta_n)-2*(R_r/R_m).^(n*p+1).*(A_zm_n+mu_0*M_theta_n)+(A_zm_n+n*p.*A_zm_n+mu_0*M_theta_n).*(R_r/R_m).^(2*n*p))./((mu_r+1)*(1-(R_r/R_s).^(2*n*p))-(mu_r-1)*((R_m/R_s).^(2*n*p)-(R_r/R_m).^(2*n*p))));

DEN_I = ((mu_r^2-1)*(-(R_i/R_r).^(2*n*p)-(R_m/R_s).^(2*n*p)+(R_r/R_s).^(2*n*p)+(R_i/R_m).^(2*n*p))+(mu_r+1)^2*(1-(R_i/R_s).^(2*n*p))+(mu_r-1)^2*((R_m/R_s).^(2*n*p).*(R_i/R_r).^(2*n*p)-(R_r/R_m).^(2*n*p)));


K_B_n = (((-A_zm_n+n*p.*A_zm_n-mu_0*M_theta_n).*((mu_r+1)-(R_i/R_r).^(2*n*p)*(mu_r-1))+(A_zm_n+n*p.*A_zm_n+mu_0*M_theta_n).*((R_r/R_m).^(2*n*p)*(mu_r-1)-(R_i/R_m).^(2*n*p)*(mu_r+1))+2*(R_r/R_m).^(n*p+1).*((A_zm_n+mu_0*M_theta_n-mu_r*n*p.*A_zm_n)+(R_i/R_r).^(2*n*p).*(A_zm_n+mu_0*M_theta_n+mu_r*n*p.*A_zm_n)))./DEN_I);


    

% (1.28e-5*B^2-7.65e-6*B+1.02e-6)*f^2+(1.71e-2*B-4.13e-3)*f
speed = [500 1000 1500 2000 2500 3000 3500 4000 4500 5000 5500];

CCM_loss_freq = zeros(1,length(speed));
CAL2_loss = zeros(1,length(speed));
VARCO_loss = zeros(1,length(speed));
VARCO_rot = zeros(1,length(speed));
for ii = 1:length(speed)
    %% Setting up data for the iron loss analysis
    n_rpm = 500*ii; % rotational speed [rpm]
    f = p*n_rpm/60; % fundamental frequency [Hz]
    f_h = f*n; % harmonc frequency components
    ...
    B_rad_n = CORR*abs(2*K_B_n./((R_s/R_se).^(2*n*p)-1).*((r_dis'/R_se).^(n*p-1).*(R_m/R_se).^(n*p+1)-(R_m./r_dis)'.^(n*p+1)));
    B_tan_n = CORR*abs(2*K_B_n./((R_s/R_se).^(2*n*p)-1).*((r_dis'/R_se).^(n*p-1).*(R_m/R_se).^(n*p+1)+(R_m./r_dis)'.^(n*p+1)));

    % Iron losses contributions with the  linear stator core discretization
    % method
    B_max_r = max(B_rad_n*cos(n'.*p.*Theta),[],2);
    B_max_theta = max(-B_tan_n*sin(n'.*p.*Theta),[],2);
    ...
    % time domain
    EC_loss_dis_rad_diff = mean(dis_weight*COEFF(1)*abs(diff((B_rad_n*cos(n'.*p.*Theta)),1,2)/(theta(2)-theta(1))).^B_exp(1)*(2*pi*f/p)^B_exp(1)/(2*pi^2));
    EC_loss_dis_tan_diff = mean(dis_weight*COEFF(1)*abs(diff((-B_tan_n*sin(n'.*p.*Theta)),1,2)/(theta(2)-theta(1))).^B_exp(1)*(2*pi*f/p)^B_exp(1)/(2*pi^2));

    EX_loss_dis_rad_diff = mean(dis_weight*COEFF(3)*abs(diff((B_rad_n*cos(n'.*p.*Theta)),1,2)/(theta(2)-theta(1))).^B_exp(3)*(2*pi*f/p)^B_exp(3)/8.7631);
    EX_loss_dis_tan_diff = mean(dis_weight*COEFF(3)*abs(diff((-B_tan_n*sin(n'.*p.*Theta)),1,2)/(theta(2)-theta(1))).^B_exp(3)*(2*pi*f/p)^B_exp(3)/8.7631);
    ...
    CCM_loss_freq(ii) = STACK*(EC_loss_dis_rad+EC_loss_dis_tan+HY_loss_dis_rad+HY_loss_dis_tan+EX_loss_dis_rad+EX_loss_dis_tan);
    ...
    if index_1>0
        n1 = n(1:index_1);
        f_h1 = f*n1; % harmonic frequency components
        ...
        % method
        % Eddy currents contribution
        B_rad_n1 = CORR*abs(2*K_B_n(1:n1)./((R_s/R_se).^(2*n1*p)-1).*((r_dis'/R_se).^(n1*p-1).*(R_m/R_se).^(n1*p+1)-(R_m./r_dis)'.^(n1*p+1)));
        B_tan_n1 = CORR*abs(2*K_B_n(1:n1)./((R_s/R_se).^(2*n1*p)-1).*((r_dis'/R_se).^(n1*p-1).*(R_m/R_se).^(n1*p+1)+(R_m./r_dis)'.^(n1*p+1)));
        ...
        B_max_r = CORR*max((2*K_B_n./((R_s/R_se).^(2*n*p)-1).*((r_dis'/R_se).^(n*p-1).*(R_m/R_se).^(n*p+1)-(R_m./r_dis)'.^(n*p+1)))*cos(n'.*p.*Theta),[],2);
        B_max_theta = CORR*max(-(2*K_B_n./((R_s/R_se).^(2*n*p)-1).*((r_dis'/R_se).^(n*p-1).*(R_m/R_se).^(n*p+1)+(R_m./r_dis)'.^(n*p+1)))*sin(n'.*p.*Theta),[],2);
    ...
    % method
    % Eddy currents contribution
    B_rad_n2 = CORR*abs(2*K_B_n(index_1+1:end)./((R_s/R_se).^(2*n2*p)-1).*((r_dis'/R_se).^(n2*p-1).*(R_m/R_se).^(n2*p+1)-(R_m./r_dis)'.^(n2*p+1)));
    B_tan_n2 = CORR*abs(2*K_B_n(index_1+1:end)./((R_s/R_se).^(2*n2*p)-1).*((r_dis'/R_se).^(n2*p-1).*(R_m/R_se).^(n2*p+1)+(R_m./r_dis)'.^(n2*p+1)));
    ...
    CAL2_loss(ii) = STACK*(CAL2_loss1 + CAL2_loss2);
    ...
    if index_1>0
        n1 = n(1:index_1);
        f_h1 = f*n1; % harmonc frequency components
    ...
    VARCO_loss(ii) = STACK*(VARCO_loss1 + VARCO_loss2);
    ...
    if index_1>0
        ...
        % time domain
        % EX_loss_dis_rad_diff = mean(l_a*rho_Fe*COEFF(3)*(pi*((r_dis+delta_r/2).^2-(r_dis-delta_r/2).^2))*abs(diff((2*K_B_n./((R_s/R_se).^(2*n*p)-1).*((r_dis'/R_se).^(n*p-1).*(R_m/R_se).^(n*p+1)-(R_m./r_dis)'.^(n*p+1))*cos(n'.*p.*Theta)),1,2)/(theta(2)-theta(1))).^B_exp(3)*(2*pi*f/p)^B_exp(3)/8.7631);
        % EX_loss_dis_tan_diff = mean(l_a*rho_Fe*COEFF(3)*(pi*((r_dis+delta_r/2).^2-(r_dis-delta_r/2).^2))*abs(diff((-2*K_B_n./((R_s/R_se).^(2*n*p)-1).*((r_dis'/R_se).^(n*p-1).*(R_m/R_se).^(n*p+1)+(R_m./r_dis)'.^(n*p+1))*sin(n'.*p.*Theta)),1,2)/(theta(2)-theta(1))).^B_exp(3)*(2*pi*f/p)^B_exp(3)/8.7631);
    ...
    VARCO_rot(ii) = STACK*(VARCO_rot1 + VARCO_rot2);
...
figure
hold on
plot(speed,VARCO_loss)
plot(speed,VARCO_rot)
plot(speed,CCM_loss_freq)
plot(speed,CAL2_loss)
legend('VARCO', 'VARCOrot', 'CCM', 'CAL2') 
